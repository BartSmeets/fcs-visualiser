...
with st.container(border=True):
    ...
    with col1:
        m1 = st.number_input(r'$m_{1}$ (amu)', min_value=0)
        t1 = st.number_input(r'$t_{1}$ (μs)', min_value=0)
    with col2:
        m2 = st.number_input(r'$m_{2}$ (amu)', min_value=0)
        t2 = st.number_input(r'$t_{2}$ (μs)', min_value=0)

# Output
with st.container(border=True):
    st.write('## Solution')
    with catch_warnings(record=True) as w:
        a, k = fsolve(system, [0.088, 0.19], args=(t1, t2, m1, m2))
